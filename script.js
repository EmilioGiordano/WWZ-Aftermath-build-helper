fetch('perks.json')
    ...
    for (let row = 0; row < 4; row++) {
        ...
        if (perk) {
            cell.className = `flex items-center justify-center rounded-full w-20 h-20 text-center font-semibold text-white shadow-lg border-4 mb-2 cursor-pointer ${perk.type === 'red' ? 'bg-red-700 border-red-900' : 'bg-gray-500 border-gray-700'}`;
